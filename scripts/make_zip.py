...
def  main():
  ...
  installer = "\installer\QuadFilteredDelay Installer.exe"
   
  if demo:
    installer = "\installer\QuadFilteredDelay Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\QuadFilteredDelay manual.pdf" 
  ]
  ...
  ZIPNAME = "QuadFilteredDelay-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "QuadFilteredDelay-v" + FULLVERSIONSTR + "-win-demo.zip"
